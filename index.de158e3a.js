...
function fireworkPositionsAndColors() {
    ...
    while(colorset.length < 3){
        ...
        if (!colorset.includes(fireworkColors[randomColorIndex])) colorset.push(fireworkColors[randomColorIndex]);
    }
    // spark coloring
    for(let i in fireworkSparks)fireworkSparks[i].map((el)=>{
    ...
    for(let i = 0; i < fireworkContainers.length; i++)root.style.setProperty(`--fw${i + 1}-offset`, `${fireworkContainers[i].offsetTop}px`);
}
fireworkPositionsAndColors(); // setInterval(() => {
